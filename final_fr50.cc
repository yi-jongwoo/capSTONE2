...
string add_error(string a){
	a[rand()%len_of_chunk+4]=rand()%256;
	a[rand()%len_of_chunk+4]=rand()%256;
	a[rand()%len_of_chunk+4]=rand()%256;
	return a;
}
void start_play_media(int);
/*
...
public:
	LinkProfile(){memset(&addr,0,sizeof addr);}
...
namespace Client{
	struct ClientMemory {
		int my_type=0; // [normal, error, freerider, forger]
		Ipv4Address my_ip;
	...
	void blame(int i){
		//cout<<'B'<<i<<endl;
		bytes c="\x01________________";
		*(ull*)&c[1]=toull(arr[i].my_ip,arr[i].my_tcp_port);
		*(ull*)&c[9]=1;
		SendTcpPacket(i,
			arr[i].server_ip,arr[i].server_tcp_port,
			c
		);
		arr[i].integrity_failures=0;
		arr[i].freerider_stack=0;
	}
		...
		if(n%5)
			...
			for(int j=0;j<n;j+=5)
				if(arr[i].chunks[x][*(uint32_t*)&code[j]]!=code[j+4])
					res=string(1,char(0));
		SendUdpPacket(i,ip,port,string(1,char(0))+payload.substr(0,4)+res);
	}
	void HandleReceivedUdpData(int i,bytes payload) {
		if(!arr[i].onstate)return;
		...
		case 0:
			if(payload[5]){
				...
				else if(arr[i].my_type==3){
					for(int j=0;j<len_of_chunk;j++)
						arr[i].chunks[x][j+4]=rand()%256;
				}
			...
			else if(++arr[i].integrity_failures<3){
				int x=*(uint32_t*)&payload[1];
				if(!arr[i].chunks[x].empty()){
					arr[i].integrity_failures=0;
					return;
		...
		case 1:
			{
				...
				if(arr[i].my_type==1){
					arr[i].tmp[x]=add_error(arr[i].tmp[x]);
				}
	...
	void HandleReceivedTcpData(int i,bytes buffer) {
		...
		if(arr[i].lp[0]==0)arr[i].onstate=false;
		arr[i].freerider_stack=0;
		arr[i].integrity_failures=0;
		if(!arr[i].playing){
			arr[i].playing=true;
	...
	void check_has_chunk(int i,int chunk_id){
		...
		if(!arr[i].onstate)return;
		if(arr[i].chunks[chunk_id].empty()){
			if((arr[i].freerider_stack+=10)>=100){
				blame(i);
			}
		}
		else{
			if((arr[i].freerider_stack-=1)<0){
				arr[i].integrity_failures=0;
			}
		}
	}
	void print_statistic(int i){
		int cnt=0;
		for(bool b:arr[i].chunkfaults)
			if(!b)cnt++;
		//cout<< arr[i].my_type <<','<<cnt<<'/'<< arr[i].chunkfaults.size()<<endl;
		ffres[arr[i].my_type] += cnt;
		ffdiv[arr[i].my_type] += arr[i].chunkfaults.size();
	}
}
...
namespace Server{
	...
	public:
		ull tcp2idx(tcp_t x){
			if(addr_idx.find(x)==addr_idx.end())
				return ull(-1);
			return addr_idx[x];
		}
		...
		std::vector<std::pair<tcp_t,LinkProfile>> deletePeer(int i){
			...
			std::vector<std::pair<tcp_t,LinkProfile>> res;
			res.emplace_back(arr[i].first,LinkProfile());
...
void server_send_chunk(bytes chunk){ // chunk includes chunk id
	...
	chunk_max=max(chunk_max,*(int*)&chunk[0]);
}
	...
	if(n%5)
		...
		for(int j=0;j<n;j+=5)
			if(chunks[x][*(uint32_t*)&code[j]]!=code[j+4])
				res=string(1,char(0));
	SendUdpPacket(0,ip,port,string(1,char(0))+payload.substr(0,4)+res);
}
...
void did_you_recived_next_chunk(int i,int chunk_id){
	if(chunk_id==num_of_chunk)
		return;
	Simulator::Schedule(
		Seconds(time_per_chunk),
		&did_you_recived_next_chunk,
		i,chunk_id+1
	);
	Client::check_has_chunk(i,chunk_id);
}
void start_play_media(int i){
	int chunk_id_now = chunk_max +1; 
	Simulator::Schedule(
		Seconds(acceptable_delay),
		&did_you_recived_next_chunk,
		i,chunk_id_now
	);
...
void setting(Ipv4InterfaceContainer ipif){
	...
	srand(clock());
	for(int j=0;j<10;j++){
		int x=rand()%peer_cnt+1;
		while(Client::arr[x].my_type)
			x=rand()%peer_cnt+1;
		Client::arr[x].my_type=2;
	}
	...
	for(int i=0;i<=peer_cnt;i++){
		...
		Client::arr[i].server_ip = ipif.GetAddress(0);
		Client::arr[i].server_tcp_port = tcpPort;
		//if(rand()%10==0)Client::arr[i].my_type = 3;
		//if(rand()%10==0)Client::arr[i].my_type = 2;
		//if(rand()%10==0)Client::arr[i].my_type = 1;
...
int main(int argc, char *argv[]) {
	...
	*/
	...
	freopen("FR0.50.csv","at",stdout);
	cout<<(double)ffres[0] / ffdiv[0]<<',';
	cout<<(double)ffres[2] / ffdiv[2]<<'\n';
	//cout<<(double)ffres[3] / ffdiv[3]<<'\n';
}
